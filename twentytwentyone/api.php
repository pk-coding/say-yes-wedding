...
// Główna tabela z zapisanym JSON-em całego kalkulatora
$wp_calculator_results = $db->query("
    CREATE TABLE IF NOT EXISTS `wp_calculator_results` (
        `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
        `user_id` BIGINT(20) UNSIGNED NOT NULL,
...
// Tabela na dynamiczne "pomysły"
$wp_calculator_ideas = $db->query("
    CREATE TABLE IF NOT EXISTS `wp_calculator_ideas` (
        `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
        `result_id` INT(11) UNSIGNED NOT NULL,
        `idea_name` VARCHAR(255) NOT NULL,
        `idea_price` DECIMAL(10,2) NOT NULL,
        PRIMARY KEY (`id`),
        FOREIGN KEY (`result_id`) REFERENCES `wp_calculator_results`(`id`) ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
");
...
if ($wp_calculator_results && $wp_calculator_ideas && $wp_user_invites) {
